            ...
            with uproot.recreate(outfile_name) as outfile:
                for data, cfg in syst_pack:
                    ...
                    for chunk in data:
                        ...
                        outfile[tree_name].extend({'predicted_class': predictions['predicted_class'], 'predicted_prob': predictions['predicted_prob']})
...
def get_predictions(model, data, variables, cfg, modify):
    ...
    if cfg['select'] != "None": data.query(cfg['select'], inplace=True) # sample is iterator - can't filter events in _getDF() so implement it here
